...
{ // Main connected loop
   ...
   if (client) { //Envio wifi + BlueTooht

    int sensorValue1 = analogRead(A0);
    int sensorValue2 = analogRead(A3);

    String packageBT = (String) sensorValue1 + "," + (String) sensorValue2; 
    
    client.println(packageBT);
    
    if (SerialBT.available()) {     
    SerialBT.println(packageBT);
     }  
delay (50);
   }
    
    if (SerialBT.available()) { //Envio solo BlueTooth
